...
def parse_option():
    ...
    # control the finetuning type
    parser.add_argument('--finetune_type', type=str, default='SLF', choices=['SLF', 'ALF', 'AFF_trades'])
...
def train(train_loader, model, classifier, net, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        if opt.finetune_type == "AFF":
            _, x_adv = net(images, labels, train=False, finetune_type="AFF")
            ...
            loss, output = trades_loss(model=model,
                                       ...
                                       trainmode=trainmode,
                                       fixmode='f1')
        elif opt.finetune_type == "AFF_trades":
            # calculate robust loss
            step_size = 2. / 255.
            epsilon = 8. / 255.
            num_steps_train = 10
            ...
            loss, output = trades_loss(model=model,
                                       ...
                                       fixmode='f1',
                                       trades=True)
        elif opt.finetune_type == "SLF":
            with torch.no_grad():
                features = model(images, return_feat=True)
            output = classifier(features.detach())
            loss = criterion(output, labels)
        else:  # ALF, use PGD examples to train the classifier
            output, _ = net(images, labels, train=False, finetune_type="ALF")
            loss = criterion(output, labels)
...
def main():
    ...
    # build optimizer
    if opt.finetune_type == "SLF" or opt.finetune_type == "ALF":
    ...
    logname = ('logger/' + '{}_linearnormal'.format(opt.name+str(opt.learning_rate)))
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            if not os.path.exists('./checkpoint/{}/'.format(opt.name+str(opt.learning_rate))):
                os.makedirs('./checkpoint/{}/'.format(opt.name+str(opt.learning_rate)))
            if opt.finetune_type == "AFF_trades":
                torch.save(state, './checkpoint/{}/best_aff.ckpt'.format(opt.name+str(opt.learning_rate), epoch))
            elif opt.finetune_type == "ALF":
                torch.save(state, './checkpoint/{}/best_alf.ckpt'.format(opt.name+str(opt.learning_rate), epoch))
            else:
                torch.save(state, './checkpoint/{}/best.ckpt'.format(opt.name+str(opt.learning_rate), epoch))
        ...
        if epoch % opt.save_freq == 0:
            ...
            if not os.path.exists('./checkpoint/{}/'.format(opt.name+str(opt.learning_rate))):
                os.makedirs('./checkpoint/{}/'.format(opt.name+str(opt.learning_rate)))
            torch.save(state, './checkpoint/{}/ep_{}.ckpt'.format(opt.name+str(opt.learning_rate), epoch))
