...
python -m torch.distributed.launch --nproc_per_node=2 --master_port 12561 \
...
python -m torch.distributed.launch --nproc_per_node=1 --master_port 12561 \
    linear_eval.py --ngpu 1 --batch-size=1024 --train_type=linear_eval --model=$2 --epoch 150 --lr 0.1 --name $1 \
    --load_checkpoint=checkpoint/ckpt.t7rocl-cifar100Rep_attack_ep_0.0314_alpha_0.007_min_val_0.0_max_val_1.0_max_iters_7_type_linf_randomstart_Truecontrastive_ResNet18_cifar-100_b256_nGPU2_l256 \
...
python -m torch.distributed.launch --nproc_per_node=1 --master_port 12351 \
...
# sh shell/rocl-cifar100.sh rocl-cifar100 ResNet18 cifar-100 | tee logs/rocl-cifar100.out
