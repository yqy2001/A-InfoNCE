...
python -m torch.distributed.launch --nproc_per_node=2 --master_port 12565 \
    rocl_train.py --ngpu 2 --batch-size=256 --model=$2 --k=7 --loss_type=sim --advtrain_type=Rep --attack_type=linf \
    --name=$1 --regularize_to=other --attack_to=other --train_type=contrastive --dataset=$3 --HN

python -m torch.distributed.launch --nproc_per_node=1 --master_port 12565 \
    linear_eval.py --ngpu 1 --batch-size=1024 --train_type=linear_eval --model=$2 --epoch 150 --lr 0.1 --name $1 \
    --load_checkpoint=checkpoint/ckpt.t7rocl-HN-cifar100-2Rep_attack_ep_0.0314_alpha_0.007_min_val_0.0_max_val_1.0_max_iters_7_type_linf_randomstart_Truecontrastive_ResNet18_cifar-100_b256_nGPU2_l256_HN \
...
python -m torch.distributed.launch --nproc_per_node=1 --master_port 12355 \
...
# sh shell/rocl-HN-cifar100.sh rocl-HN-cifar100 ResNet18 cifar-100 | tee logs/rocl-HN-cifar100.out
