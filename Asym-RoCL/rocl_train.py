...
from utils import progress_bar, checkpoint, checkpoint_train, AverageMeter, accuracy

from loss import pairwise_similarity_train, NT_xent, NT_xent_HN
...
if 'Rep' in args.advtrain_type:
    ...
    Rep = RepresentationAdv(model, projector, epsilon=args.epsilon, alpha=args.alpha, min_val=args.min, max_val=args.max, max_iters=args.k, args=args, _type=args.attack_type, loss_type=args.loss_type, regularize = args.regularize_to)
else:
...
def train(epoch):
    ...
    for batch_idx, (ori, inputs_1, inputs_2, label) in enumerate(trainloader):
        ...
        similarity, similarity_grad, gathered_outputs = pairwise_similarity_train(outputs, args, temperature=args.temperature, multi_gpu=multi_gpu, adv_type = args.advtrain_type) 
        
        if args.HN:
            simloss  = NT_xent_HN(similarity, args.advtrain_type, args)
        else:  
            simloss  = NT_xent(similarity, args.advtrain_type, args)
...
def test(epoch, train_loss):
    ...
    if epoch == args.epoch - 1 and args.local_rank % ngpus_per_node == 0:
        checkpoint_train(model, train_loss, epoch, args, optimizer)
        checkpoint_train(projector, train_loss, epoch, args, optimizer, save_name_add='_projector')
       
    # Save at every 100 epoch #
    elif epoch % 100 == 0 and args.local_rank % ngpus_per_node == 0:
        checkpoint_train(model, train_loss, epoch, args, optimizer, save_name_add='_epoch_'+str(epoch))
        checkpoint_train(projector, train_loss, epoch, args, optimizer, save_name_add=('_projector_epoch_' + str(epoch)))
