...
from utils import progress_bar, checkpoint_train
...
if args.epochwise:
    for k in range(100,1000,100):
        ...
        checkpoint_train(model, test_acc, args.epoch, args, loptim, save_name_add='epochwise'+str(k))
        checkpoint_train(linear, test_acc, args.epoch, args, loptim, save_name_add='epochwise'+str(k)+'_linear')
        if args.local_rank % ngpus_per_node == 0:
...
checkpoint_train(model, test_acc, args.epoch, args, loptim)
checkpoint_train(linear, test_acc, args.epoch, args, loptim, save_name_add='_linear')
